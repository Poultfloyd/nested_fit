...
class Analysis(object):
    ...
    def load_data(
        ...
        path=currentpath,
        do_load_input=True,
        do_load_output_results=False,
        do_load_output_data=True,
    ):
    ...
    def load_output_data(self, path=currentpath):
        ...
        self.df = pd.read_csv(
            ...
            header=0,
            names=["weight", "lnlikelihood"]
            + ["val_%s" % d for d in range(1, self.number_of_values + 1)],
        )

        self.df = self.df.rename(
            columns={
                "val_%d" % np: self.input_data["par_name"][np - 1].replace("'", "")
                for np in self.input_data["par_n"]
            }
        )
    ...
    def read_input(self, path=currentpath):
        ...
        input_data["par_n"] = [
            int(lines[16 + index].split()[0]) for index in range(npar)
        ]
        input_data["par_name"] = [lines[16 + index].split()[1] for index in range(npar)]
        input_data["par_value"] = [
            float(lines[16 + index].split()[2]) for index in range(npar)
        ]
        input_data["par_step"] = [
            float(lines[16 + index].split()[3]) for index in range(npar)
        ]
        input_data["par_min"] = [
            float(lines[16 + index].split()[4]) for index in range(npar)
        ]
        input_data["par_max"] = [
            float(lines[16 + index].split()[5]) for index in range(npar)
        ]
        input_data["par_fixed"] = [
            int(lines[16 + index].split()[6]) for index in range(npar)
        ]
    ...
    def read_output(self, path=currentpath):
        ...
        if input_data["nlive"] != output_data["nlive_out"]:
            print(
                "input nlive", input_data["nlive"], "output nlive", output_data["nlive"]
            )
            sys.exit("Check your input/output files")
        ...
        # Max parameter set
        output_data["max"] = [
            float(lines[10 + index].split()[1]) for index in range(npar)
        ]
        ...
        output_data["mean"] = [
            float(lines[12 + npar + index].split()[1]) for index in range(npar)
        ]
        output_data["sd"] = [
            float(lines[12 + npar + index].split()[3]) for index in range(npar)
        ]
        ...
        output_data["conf_level_m99"] = [
            float(lines[14 + 2 * npar + index].split()[1]) for index in range(npar)
        ]
        output_data["conf_level_m95"] = [
            float(lines[14 + 2 * npar + index].split()[2]) for index in range(npar)
        ]
        output_data["conf_level_m68"] = [
            float(lines[14 + 2 * npar + index].split()[3]) for index in range(npar)
        ]
        output_data["conf_level_p68"] = [
            float(lines[14 + 2 * npar + index].split()[5]) for index in range(npar)
        ]
        output_data["conf_level_p95"] = [
            float(lines[14 + 2 * npar + index].split()[6]) for index in range(npar)
        ]
        output_data["conf_level_p99"] = [
            float(lines[14 + 2 * npar + index].split()[7]) for index in range(npar)
        ]

        # Median of parameters
        output_data["median"] = [
            float(lines[14 + 2 * npar + index].split()[4]) for index in range(npar)
        ]
    ...
    def plot(
        ...
        if nset < 1:
            ...
            data = loadtxt(
                self.path + "nf_output_data_" + typeof + "_" + str(nset) + ".dat"
            )
        ...
        if high_definition:
            ...
            if nset < 1:
                ...
                data_fit = loadtxt(
                    self.path + "nf_output_fit_" + typeof + "_" + str(nset) + ".dat"
                )
        ...
        if logscale:
            plt.yscale("log")
        plt.errorbar(
            x, y, yerr=sy, xerr=None, fmt="or", ecolor="red", mec="red", **linestyle
        )
        ...
        if xmin == 0 and xmax == 0:
            ...
            plt.xlim([xmin, xmax])
        plt.errorbar(
            x, r, yerr=sy, xerr=None, fmt="ob", ecolor="blue", mec="blue", **linestyle
        )
        plt.errorbar(
            [minx, maxx], [0.0, 0.0], yerr=None, xerr=None, fmt="-k", **linestyle2
        )
    ...
    def plot2D(
        ...
        afit = genfromtxt(
            self.path + "nf_output_fit_" + typeof + "_2D.dat", skip_header=1
        )
        ares = genfromtxt(
            self.path + "nf_output_fitres_" + typeof + "_2D.dat", skip_header=1
        )
    ...
    def plot2D_projection(
        ...
        # Built histogram
        h, edges = histogram(
            xxxx[(zzz > 0) & (xxx >= minx) & (xxx <= maxx)],
            weights=zzz[(zzz > 0) & (xxx >= minx) & (xxx <= maxx)],
            bins=bins,
        )
        ...
        plt.errorbar(
            [xmin, xmax], [0.0, 0.0], yerr=None, xerr=None, fmt="-k", **linestyle2
        )
        plt.tight_layout()
    ...
    def histo(
        ...
        # Select range
        if xmin == None:
            xmin = data[:, par_index].min()
        if xmax == None:
            xmax = data[:, par_index].max()
    ...
    def plot_par(self, par_number, path=currentpath):
        ...
        plt.scatter(ix, data[:, par_index], c=data[:, 0], linewidth=0.0, cmap=cmap)
        cbar = plt.colorbar()
    ...
    def plot_clusters(
        ...
        if label == None:
            ...
            label = (
                available_files[-1].split("_")[4]
                + "_"
                + available_files[-1].split("_")[5].split(".")[0]
            )
        ...
        # Select the data
        if xmin == None:
            xmin = data[:, par_index1].min()
        if xmax == None:
            xmax = data[:, par_index1].max()
        if ymin == None:
            ymin = data[:, par_index2].min()
        if ymax == None:
            ymax = data[:, par_index2].max()
        data = data[
            (data[:, par_index1] < xmax)
            & (data[:, par_index1] > xmin)
            & (data[:, par_index2] < ymax)
            & (data[:, par_index2] > ymin)
        ]
        ...
        for i in range(ncl):
            plt.plot(
                data[data[:, 0] == i][:, par_index1],
                data[data[:, 0] == i][:, par_index2],
                "o",
            )
        plt.tight_layout()
    ...
    def plot_clusters3D(
        ...
        if ymax == None:
            ymax = data[:, par_index2].max()
        if zmin == None:
            zmin = data[:, par_index3].min()
        if zmax == None:
            zmax = data[:, par_index3].max()
        data = data[
            (data[:, par_index1] < xmax)
            & (data[:, par_index1] > xmin)
            & (data[:, par_index2] < ymax)
            & (data[:, par_index2] > ymin)
            & (data[:, par_index3] < zmax)
            & (data[:, par_index3] > zmin)
        ]
    ...
    def histo2D(
        ...
        # Make histogram
        histo2D, yedges, xedges = histogram2d(
            data[:, par_index2], data[:, par_index1], bins=bins, weights=data[:, 0]
        )
    ...
    def triangle_plot(
        ...
        Args:
            ...
            savefile (bool, optional): Option for saving the triangle plot. Saved under currentpath/triangle_plot.png. Defaults to False.
        """
    ...
    def add_simulations(self, directories, labels, prefix=""):
        ...
        for dir in directories:
            ...
            for k in input_types:
                if k in inputs_par:
                    for p in parameters:
                        summary[k + "_" + p].append(
                            dir_input[k + "_" + p]
                            if k + "_" + p in dir_input
                            else np.nan
                        )
                else:
        ...
        sum = pd.DataFrame(
            dict([(k, pd.Series(v)) for k, v in summary.items()])
        ).set_index("labels")
